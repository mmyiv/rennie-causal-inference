...
```{r}
# Turkey HSD for fall
turkey_fall <- TukeyHSD(fall_model)
...
-   375:treatment vs. 373:treatment
    ...
    ```{r}
    fall_residuals <- residuals(fall_model)
    fall_fitted <- fitted(fall_model)  # Predicted values
    qqnorm(fall_residuals)
    qqline(fall_residuals, col = "red")
